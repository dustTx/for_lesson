...
if [[ $memnum -gt 0 ]]; then
	...
	count=$[ $count + 1 ]
	eval $(ps -aux -q $i -h | awk -v num=$count \
	    '{if($4 < 50) {exit} else {printf("memproname["num"]=%.40s;\
	    memproid["num"]=%d;memprouser["num"]=%s;memprocpu["num"]=%s;\
	    mempromem["num"]=%s;",$11,$2,$1,$3,$4);} } \
...
if [[ $cpuchecknum -gt 0 ]]; then
	...
	${cpuprocpu[$i]}% ${cpupromem[$i]}%"
    done
fi

if [[ $memchecknum -gt 0 ]]; then
    for (( i = 1; i <= $memchecknum; i++ )); do
	echo "$NowTime ${memproname[$i]} ${memproid[$i]} ${memprouser[$i]} \
	${memprocpu[$i]}% ${mempromem[$i]}%"
    done
